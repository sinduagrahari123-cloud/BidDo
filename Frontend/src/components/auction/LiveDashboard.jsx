...
function LiveDashboard({ auction, refreshTrigger }) {
  ...
  const participants = auction.members.filter(
    (m) => m.role === 'participant' && m.status === 'approved'
  );
  ...
  // ======================================================
  const currentBidder = auction.members.find(
    (member) =>
      member.userId?._id?.toString() ===
      auction.currentBid?.bidderId?.toString()
  );
    ...
    <div className="mt-8 space-y-6">
      ...
      {auction.currentItem && (
        <div className="auction-hero relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl p-6 flex flex-col items-center text-center">
          ...
          {/* ==================================================
              HIGHEST BIDDER TEAM NAME
              ================================================== */}
          {auction.currentBid?.amount > 0 && (
            <p className="mt-2 text-sm md:text-base text-slate-300">
              Highest Bid By:{' '}
              <span className="font-bold text-green-400">
                {teamName(currentBidder)}
              </span>
            </p>
          )}
